 ...
 * 
 * Provides file storage operations using Cloudflare R2 (S3-compatible API).
 * 
 * @example Basic usage
 * ```typescript
...
export class R2Provider implements IStorageProvider {
  readonly name = StorageProvider.R2;
  private readonly client: any;
  private readonly bucket: string;
  private readonly logger: Logger;

  /**
   * Creates a new R2Provider instance
   * 
   ...
   * ```
   */
  constructor(config: R2Config, logger: Logger = console) {
    this.logger = logger;
    ...
    if (!config.bucket) {
      ...
      throw new StorageError('R2 bucket is required', this.name);
    }

    if (!config.accountId) {
      this.logger.error('Basepack Storage: Provider account ID missing', { provider: this.name });
      throw new StorageError('R2 account ID is required', this.name);
    }

    if (!config.credentials) {
      this.logger.error('Basepack Storage: Provider credentials missing', { provider: this.name });
      throw new StorageError('R2 credentials are required', this.name);
    }
   ...
   * 
   * Downloads the file from the URL and uploads it to R2.
   * 
   ...
   * 
   * Creates a pre-signed URL that allows temporary access to an R2 object.
   * 
   ...
   * 
   * Performs a lightweight operation to verify connectivity to R2.
   * 
   ...
   *   console.log(`R2 is healthy (${health.responseTime}ms)`);
   * } else {
   *   console.error(`R2 is unhealthy: ${health.error}`);
   * }
   * ```
   */
