/**
 * Retry utilities for messaging operations
 * @module messaging/retry
 */
  ...
  for (let attempt = 0; attempt <= opts.retries; attempt++) {
    try {
      ...
      if (attempt === opts.retries) {
        logger.debug('Basepack Messaging: Max retries reached', { attempt: attempt + 1, error });
        break;
      }

      // Don't retry if error is not retryable
      if (!isRetryableError(error)) {
        logger.debug('Basepack Messaging: Error not retryable', { error });
        break;
      }

      // Calculate delay and wait
      const delay = calculateDelay(attempt, opts);
      logger.info('Basepack Messaging: Retrying operation', {
        attempt: attempt + 1,
